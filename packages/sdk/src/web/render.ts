...
// DOM renderer for the Web Component modal. It mirrors the React modal's
// markup but builds elements imperatively. The logic lives in the shared
// controller; only this presentation layer differs from React.
...
export function renderModal(state: CheckoutState, handlers: ModalHandlers): HTMLElement {
  ...
  const closeBtn = make("button", "close", "x");
...
function body(state: CheckoutState, h: ModalHandlers): HTMLElement[] {
  switch (state.status) {
    ...
    case "success": {
      const center = make("div", "center");
      const icon = make("div", "successIcon", "OK");
